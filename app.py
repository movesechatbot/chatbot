...
from followup_worker import run_followup_cycle
...
def docs_update():
    try:
        ...
        if goal_reached:
            followup.mark_goal(user_id, True)
...
def chat():
    ...
    t0 = time.time()
    ctx = None
    bot_replied = False  # blindagem para evitar múltiplos track_bot_reply

    def reply_and_mark(payload, status=200):
    ...
    try:
        ...
        # 🔑 USUÁRIO FALOU → RESET DO FOLLOW-UP
        followup.mark_user_reply(user_id)
        ...
        if nova_etapa == FILTRAR_CIDADE:
            cidade_status = "VALIDA" if cidade_valida(pergunta) else "INVALIDA"
        ...
        if use_semantic and best_score >= HIGH:
            ...
            if is_creci_question(pergunta):
                ...
                if creci_txt:
                    ans = limpa_negacoes_creci(f"{creci_txt}\n\n{ans}")
            ...
            if debug and llm_trace:
                resp["trace"] = llm_trace

            return reply_and_mark(resp)
        ...
        try:
            res = ask_chatgpt(
                ...
                ctx,
                history=hist,
                playbook_snippet=combined_snippet,
                debug=debug
            )
...
if __name__ == "__main__": 
    ...
    # nível de log do APScheduler
    logging.getLogger("apscheduler").setLevel(logging.INFO)

    # inicia scheduler
    scheduler.start()
    app.logger.info("Scheduler de follow-up iniciado")
